...
multinom.events <- function(var, event.rates, tau) {
  null.prob <- exp(-sum(event.rates)*tau) # Probability that no events take place
  event.probs <- rep(NA, length(event.rates)) # Initialize vector to hold event probabilities
  for (i in 1:length(event.rates)) {
    # Calculate probabilities for each event
    event.probs[i] <- (1 - null.prob) * (event.rates[i]/sum(event.rates))
  }
  # Concatenate probability of nothing happening with other prob. for events
  event.probs <- c(null.prob, event.probs)
  #browser()
...
# SIR model using tau-leap method for stochastics
multinomtau.sir <- function(vars, params, tau) {
  beta <- params[1]
  gamma <- params[2]
  mu <- params[3]
...
for (t in 1:(length(t_range)-1)) {
  ...
  t <- t + 1
  tmp <- multinomtau.sir(t.vars, params, tau = i)
